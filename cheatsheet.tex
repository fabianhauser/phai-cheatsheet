...
\paragraph{3 Dimensionen} \hfill \\
	\begin{multicols}{2}
		\includegraphics[width=\linewidth]{img/darstellungKraefte.png}
...
\subsection{Drehmoment}
	\begin{multicols}{2}
		...
		\begin{align*}
			M &= r \times \vec{F} \\
			\left| M \right| &= \left| r \right| \left| \vec{F} \right| \sin \varphi
		\end{align*}
...
\[
F_{tot} = \sum_n{F_n} =\sum_n{F_{x,n}} = \sum_n{F_{y,n}} = \sum_n{F_{z,n}} = 0
\]
...
\subsection{Winkelgeschwindigkeit und Radialbeschleunigung}
	...
	\begin{tabu} {l X}
		Periode & $T = \left[ \frac{\text{Umdrehungen}}{s}\right]$ \\ %TODO: Stimmt das?
		Frequenz & $f = T^{-1}$ \\
		Winkelgeschwindigkeit & $\vec{\omega} =\frac{\omega}{r} = \left[ \frac{rad}{s} \right] = 2\pi f$ \\
		Drehgeschwindigkeit & $v = \omega r$ \\
		Winkelbeschleunigung & $\alpha$ \\ %TODO: Definition?
